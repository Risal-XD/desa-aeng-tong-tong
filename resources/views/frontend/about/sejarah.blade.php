    ...
    <section class="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        @if ($village?->history?->history_content)
            <div class="grid gap-10 lg:grid-cols-3">
                <div class="lg:col-span-2">
                    <div class="prose prose-ink max-w-none rounded-2xl border border-ink-200 bg-white p-6 text-sm leading-relaxed text-justify !text-black shadow-sm sm:p-8">
                ...
                <aside class="space-y-4">
                    <div class="rounded-2xl border border-ink-200 bg-white p-6 shadow-sm">
                        <h3 class="text-sm font-semibold uppercase tracking-wider !text-black">Informasi</h3>
                        <dl class="mt-4 space-y-3 text-sm">
                            @if ($village->history->founder_name)
                                <div>
                                    <dt class="!text-black">Pendiri / Tokoh</dt>
                                    <dd class="mt-0.5 font-medium !text-black">{{ $village->history->founder_name }}</dd>
                                </div>
                            @endif
                            @if ($village->history->founded_year)
                                <div>
                                    <dt class="!text-black">Berdiri Sejak</dt>
                                    <dd class="mt-0.5 font-medium !text-black">{{ $village->history->founded_year }}</dd>
                                </div>
                            @endif
                        </dl>
                    </div>
                    ...
                    <div class="rounded-2xl bg-brand-50 p-6">
                        <h3 class="text-sm font-semibold uppercase tracking-wider !text-black">Berlanjut</h3>
                        <p class="mt-2 text-sm !text-black">Pelajari visi dan misi pembangunan desa selanjutnya.</p>
        ...
        @else
            <div class="rounded-2xl border border-ink-200 bg-white p-10 text-center shadow-sm">
                <p class="text-sm !text-black">Konten sejarah desa belum tersedia.</p>
            </div>
        @endif
    </section>
@endsection
