...
@section('content')
    <x-admin.page-header title="Wisata" description="Kelola destinasi wisata desa.">
        @can('create', App\Models\TourismDestination::class)
            <x-slot:actions>
                <a href="{{ route('admin.economy.tourism.create') }}" class="rounded-lg bg-brand-500 px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-600">
                    + Tambah Wisata
                </a>
            </x-slot:actions>
        @endcan
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.economy.tourism.edit', $item) }}" class="rounded-md border border-ink-200 px-3 py-1.5 text-xs font-semibold text-ink-700 transition hover:bg-ink-50">
                                        Edit
                                    </a>
                                    @can('delete', $item)
                                        <x-admin.delete-form :action="route('admin.economy.tourism.destroy', $item)" label="Hapus" />
                                    @endcan
                                </div>
                            </td>
                        </tr>
