...
@section('title', 'Tambah Mpu')

@section('content')
    <x-admin.page-header title="Tambah Mpu" description="Tambahkan data Mpu/empu keris." />

    <form method="POST" action="{{ route('admin.economy.keris.store') }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <x-admin.card title="Detail Mpu">
                    <div class="space-y-4">
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <x-admin.input name="name" label="Nama Mpu" required />
                            <x-admin.input name="title" label="Gelar" placeholder="Mpu, Empu, ..." />
                        </div>
                        <x-admin.input name="slug" label="Slug" hint="Otomatis diisi dari nama bila dikosongkan." />
                        <x-admin.input name="specialties" label="Keahlian" placeholder="Pamor, Bilah, Warangka, ..." hint="Pisahkan dengan koma." />
            ...
            <div class="space-y-6">
                <x-admin.card title="Media">
                    <x-admin.file-input name="photo" label="Foto" hint="JPG/PNG/WebP, maks 4 MB" />
                </x-admin.card>

                <x-admin.card title="Prestasi & Kontak">
                    <div class="space-y-4">
                        <x-admin.input name="experience_years" label="Lama Berkarya" placeholder="30 tahun" />
                        <x-admin.input name="award" label="Penghargaan" />
