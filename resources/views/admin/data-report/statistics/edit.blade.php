...
@section('content')
    <x-admin.page-header title="Edit Statistik" description="Perbarui data statistik desa." />

    <form method="POST" action="{{ route('admin.data-report.statistics.update', $statistic) }}">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div class="lg:col-span-2">
                <x-admin.card title="Detail Statistik">
                    <div class="space-y-4">
                        <x-admin.input name="name" label="Nama Statistik" :value="$statistic->name" required placeholder="Statistik Kependudukan" />
                        <x-admin.input name="slug" label="Slug" :value="$statistic->slug" hint="Otomatis diisi dari nama bila dikosongkan." />
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <x-admin.select
                                ...
                                :selected="old('category', $statistic->category->value)"
                                required
                            />
                            <x-admin.input name="year" label="Tahun" type="number" :value="old('year', $statistic->year)" required />
                        </div>
                        ...
                        <div>
                            ...
                            >{{ old('description', $statistic->description) }}</textarea>
                            @error('description')
                ...
                <x-admin.card title="Data Statistik (baris)">
                    <div class="space-y-3" x-data="populationRows(@js($statistic->populationStatistics->map(fn ($row) => ['label' => $row->label, 'value' => $row->value, 'unit' => $row->unit])->all()))">
            ...
            <div class="space-y-6">
                <x-admin.card title="Pengaturan">
                    <x-admin.checkbox name="is_active" label="Aktif" :checked="$statistic->is_active" />
                </x-admin.card>
