...
@section('content')
    <x-admin.page-header title="Pengumuman" description="Kelola pengumuman desa.">
        @can('create', App\Models\Announcement::class)
            <x-slot:actions>
                <a href="{{ route('admin.content.announcements.create') }}" class="rounded-lg bg-brand-500 px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-600">
                    + Tambah Pengumuman
                </a>
            </x-slot:actions>
        @endcan
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.content.announcements.edit', $item) }}" class="rounded-md border border-ink-200 px-3 py-1.5 text-xs font-semibold text-ink-700 transition hover:bg-ink-50">
                                        Edit
                                    </a>
                                    @can('delete', $item)
                                        <x-admin.delete-form :action="route('admin.content.announcements.destroy', $item)" label="Hapus" />
                                    @endcan
                                </div>
                            </td>
                        </tr>
